...
Everything uses Minkowski metric (+,+,+,-) with c = 1. The core algorithms:

- **Rendering**: each object is drawn where the observer's past light cone intersects its world line. This is solved analytically as a quadratic equation per world line segment.
- **Laser hit detection**: a laser is a lightlike world line. Hit detection finds the closest spatial approach between the laser trajectory and each target world line segment, checked by the host.
- **Causality guard**: if you are inside another player's future light cone, your controls freeze -- acting on their current position would be superluminal.
- **Causal-delay notifications**: kill and spawn events are queued and displayed only when light from them reaches you.
- **Lighthouse AI intercept**: solves the relativistic intercept problem (quadratic in proper time) to aim where the target *will be*. Perfect against inertial targets; dodgeable by accelerating.
- **Rest frame view**: applies a Lorentz boost matrix to the entire THREE.js scene.

For full derivations and equations, see [`2+1/README.md`](2+1/README.md#relativistic-algorithms).
...
ミンコフスキー計量 (+,+,+,-), c = 1 で統一。主要なアルゴリズム:

- **描画**: 各オブジェクトは、観測者の過去光円錐が世界線と交わる位置に描画される。世界線セグメントごとに二次方程式を解析的に解く
- **レーザー当たり判定**: レーザーは光的世界線。ターゲット世界線セグメントとの最近接距離を求め、ホストが権威的に判定
- **因果律ガード**: 他プレイヤーの未来光円錐内にいると操作凍結（その位置情報に基づく行動は超光速情報伝達になるため）
- **因果律遅延通知**: キル・スポーンイベントは、光が観測者に届いた瞬間に表示
- **Lighthouse AI 照準**: 相対論的迎撃問題（固有時間の二次方程式）を解いてターゲットの未来位置に照準。慣性運動する敵には必中、加速で回避可能
- **静止系表示**: ローレンツブースト行列を THREE.js シーン全体に適用

数式を含む詳細な導出は [`2+1/README.md`](2+1/README.md#relativistic-algorithms) を参照。
