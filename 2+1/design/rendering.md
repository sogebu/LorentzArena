...
**物理的整合性 (2026-04-22 確認、形状変更不要で確定)**: throat=0.05 → exit=0.22 で外に広がる bell 形状は de Laval (収束-拡散) ノズルの **拡散部 (diverging bell)** の外形そのもの。直感的には「広がる形だと噴流が収束しないのでは」と疑えるが、**超音速流では面積増大 = 速度増大** (亜音速と逆の関係)。throat で Mach 1 に到達した流れが拡散部で圧力→運動エネルギー変換されて **さらに加速**するのが de Laval の設計意図で、「噴流を収束させる形状」ではない。実機ロケットエンジン (SSME / Raptor / F-1) もすべて外観はベル型拡張部で、収束部は内部に埋蔵。本実装は throat 手前の収束部を hull 内部に抽象化した同型の省略で、物理的に正当。raster flare 状の ExhaustCone がさらに外に広がる描写も、真空 under-expansion plume の自由膨張として自然。→ "fictional sci-fi nozzle" と割り切る必要なし、現設計 = de Laval 後半部の literal 模倣で一貫。
...
これらが history 長 N に比例。N=5000 では世界時刻蓄積で GC + JS execution が 16ms 予算を食い潰す。
...
**動機**: Thrust energy (§物理) により drifter は燃料で封じられ物理的 drift 制御は不要だが、戦闘領域を空間的に可視化するガイドが欲しいとの要望で un-defer。実装は視覚ガイドのみで物理判定なし (壁で跳ね返す・外に出たら死亡等は検討せず)。§物理「Thrust energy」の `R_HORIZON` 仮置き 30 と異なり、画面内で見える `ARENA_RADIUS = 40` (= LASER_RANGE × 4、 2026-04-28 に 20 から bump) を採用。

**2026-04-28 update**: open_cylinder mode の default 視覚は **正四角柱** (= SquareArenaRenderer、 半幅 ARENA_RADIUS) に変更。 旧円柱は `#shape=cylinder` 隠しオプション。 旧円柱と光円錐がどちらも円形で「壁の裏に居る」 感が出にくかったための対応。 以下「アリーナ円柱」 の記述は隠しオプション側 (= CylinderArenaRenderer) の挙動として保持。
