...
// 75 度 = 5π/12。 camera position の time component = `distance·sin(pitch)`、 spatial =
// `distance·cos(pitch)` なので `tan(pitch) > 1` (= pitch > 45 度) で camera は観測者の
// 未来光円錐内 (= 時間的に未来側) に居る扱いになる。 75 度は光円錐 45 度を大きく上回り、
// ほぼ真上からの俯瞰でアリーナ全体 + 過去世界線群が広く見える (odakin 指定 2026-04-28)。
export const DEFAULT_CAMERA_PITCH = (5 * Math.PI) / 12;
