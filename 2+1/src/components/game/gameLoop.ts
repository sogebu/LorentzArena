...
export function processPlayerPhysics(
  ...
  // Bug 14 完全治療 (2026-05-06 plan: bug14-global-active-time §6.5 post-deploy implicit Euler refactor):
  //
  // 旧 explicit Euler + substep workaround を撤廃、 **semi-implicit Euler** で friction を
  // 任意 dτ で unconditionally 安定化。 `evolvePhaseSpace` の 4 番目引数 `frictionCoefficient`
  // を渡すと内部で `newU = (u + a_world × dτ) / (1 + γkΔ)` の closed-form 1 step 計算、
  // substep loop / `MAX_STABLE_SUB_DTAU` constant 不要、 任意 dτ で発散しない。
  //
  // 数値解析根拠 (詳細: `claude-config/conventions/scientific-computing.md §2`):
  // explicit Euler は `Δ < 2/k` 制約 (= friction 安定境界) を持つが、 implicit Euler は
  // unconditionally stable。 旧 substep は「explicit を温存して dτ を分割する数値 workaround」、
  // implicit Euler は「friction の数値不安定性自体を消す」 L5 root level の fundamental fix。
  //
  // alpha (= 表示専用、 thrust 由来の world-frame 4-acceleration) は `evolvePhaseSpace` 内部で
  // friction を含めず boost(u) × (0, thrust, 0) として返るため、 旧コードの「thrust のみ boost で
  // 上書き」 の post-process が不要に。 静止漂流時の矢印反転問題も自動的に消える。
  const evolved = evolvePhaseSpace(
    me.phaseSpace,
    thrustAcceleration,
    dTau,
    FRICTION_COEFFICIENT,
...
export function processLighthouseAI(
  ...
  // future LH motion variant で friction を導入する場合は `evolvePhaseSpace` の 4 番目
  // 引数 `frictionCoefficient` を渡せば任意 dτ で unconditionally 安定 (= implicit Euler、
  // processPlayerPhysics と同 mechanism)。
