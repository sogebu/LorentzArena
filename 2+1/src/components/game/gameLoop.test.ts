...
// Bug 14 完全治療 (2026-05-06 plan: bug14-global-active-time §2.1): processPlayerPhysics
// の内部 substep が任意 dTau で integrator を unconditionally stable にすることを test。
// 旧仕様 (= substep 無し) は `dτ > 2/k = 4 sec` で `newU = u(1-kΔ)` が発散、 mobile
// 12.5h suspend 復帰時の self.pos.t = 20.37M sec runaway を生んだ root cause
// (= live capture: repro/2026-05-06-bug14-state/)。
...
describe("processPlayerPhysics substep stability (Bug 14 plan §2.1)", () => {
  ...
  it("巨大 dTau (= 45000 sec ≈ 12.5h、 N=450000): 旧仕様で発散する範囲だが substep で収束", () => {
    ...
    // 1) u は friction で完全減衰、 旧仕様の `1.6 * -22499 = -35998` 発散と全く違う
    // 解析解: u(45000) = 1.6 × exp(-0.5 × 45000) ≈ exp(-22500) → ≈ 0 (denormalized)
    expect(Math.abs(result.newPhaseSpace.u.x)).toBeLessThan(1e-100);

    // 2) pos.t は γ 積分の有界解、 ~ dTau (= γ→1 quickly) のオーダー、
    //    旧仕様の 1.6 × 10^9 オーダーの runaway とは比較にならない
