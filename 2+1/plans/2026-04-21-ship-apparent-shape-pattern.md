...
- **底面 (xy) を楕円化して奥行き feel を足す (manga stylization)**: 現 v1 は base ring の各頂点を x_∥ 方向に時間 tilt する (= 物理的過去光円錐接平面) が、見ての通り「底が撓んだ塔」印象を生みかねない。代替 / 追加案として、時間 tilt を 0 にして xy 平面内で x_∥ 方向に **ellipse 的に** 頂点を引き伸ばすのを検討。perspective foreshortening 風の奥行きヒント化。物理整合は捨てるが "manga" であれば OK。スケッチ: row 2 の `(x_∥.x, x_∥.y, 1)` → `(0, 0, 1)` に戻し、row 0/1 を `I + (k−1) x_∥ ⊗ x_∥^T` で x_∥ 方向に k 倍 stretch (k は ρ 依存 or 定数)。tilt 版との A/B 比較で採否判断。
