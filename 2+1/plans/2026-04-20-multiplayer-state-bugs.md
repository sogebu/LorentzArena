...
### A. 症状 3 → 最小表面止血 (低リスク、即効) — **完了 `2be56b4` / 再発 fix 別 commit**
...
**再発 (2026-04-20 昼 post-deploy)**: fallback + reactive 化しても撃破数リストに
`id.slice(0, 6)` が出た。**真の root cause**: `RelativisticGame.tsx` の intro 送信は
`peerManager.onMessage` 登録時の 1 回 broadcast のみ。**送信時点で開いている connection
にしか届かない**ため、後から接続した peer には永久に届かない。displayNames map に
entry が入らない状態では fallback chain のどの段でも拾えない。

**fix**: connection watcher (`prevConnectionIdsRef` diff) で検出した新規接続 peer に
対し、全 peer が自分の intro を unicast 再送信する。これで A ⇒ B や B ⇒ A の
接続順序に依存せず、双方向に displayName が伝播する。
