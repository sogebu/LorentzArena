...
A (初版 `2be56b4` + 再発 fix `e9171c4`) / B (`8ce595f`) は 2026-04-20 昼に修正 + deploy 済
(最新 deploy `e9171c4` / build `2026/04/20 10:52:08`)。
...
3. **症状 5 (host migration & タブ復帰で相手が消える)** + B' (LIVE 消失) を並行調査。
   最有力は `RelativisticGame.tsx:181-197` の切断 player 削除ロジックが migration 中の
   connections 欠落で発火、players map から他 peer が一時蒸発するケース。grace period
   付き removal を検討
