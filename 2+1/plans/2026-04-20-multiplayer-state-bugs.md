...
## Refactor — 2026-04-21 夜 (`f93eb37`、未 deploy)

Stage 1/1.5/2/3 + audit 足し算後のコード対称性を odakin 要請で review、2 helper を
PeerProvider.tsx 内で抽出。思想の対称性がコードに反映されるようになった。

- **`transferLighthouseOwnership(newOwnerId)`**: `assumeHostRole` (BH 就任時、駆動権
  取得) と `performDemotion` (降格時、駆動権放出) で完全同一だった LH.ownerId 更新
  loop を集約。対称操作が 1 行呼び出しで書ける。idempotent + immutable 更新。
- **`discoverBeaconHolder({...callbacks})`**: `demoteToClient` (beacon-acquire effect
  内) と Stage 2 `runProbe` で structurally 同一だった「使い捨て probe PeerManager →
  open → connect roomPeerId → redirect 受信 → cleanup」パターンを統合。内部 `done`
  flag で late callback の one-shot semantics を保証 (各呼び出しで独立 closure → probe
  1 の late callback が probe 2 を誤爆できない構造)。旧外部 stale guard (`probePm !== pm`
  を Stage 2 の 3 handler 冒頭で check) が helper 内の done check 1 箇所に集約。
- `performDemotion` doc に `assumeHostRole` との対称操作表 (clearBH↔broadcast
  redirect、setAsBH↔setBHId+connect、transferLHOwner(self)↔transferLHOwner(real)、
  roleVersion bump 共通) を markdown table で追加、思想対称性がコメントから読める。

Pure refactor、typecheck + Vitest 58/58 pass、net +159/-146 (helper docs 増、logic
重複減)。動作変化無しのため未 deploy — 本番は Stage 3 fix 含む `1b9e743` のまま。

## 次セッションで最初にやること (改訂)

**Stage 1 / 1.5 / 2 / 3 すべて deploy 済** (2026-04-21、`1b9e743` / build 08:20:46)、
以降 `f93eb37` で code 対称性 refactor 済 (未 deploy)。マルチプレイ state バグ 5 症状
全て修正済み marker 付き。次の作業候補:
...
## Stage 2 設計 — 2026-04-21 (実装・deploy 済 `f8a4589` / build 08:02:51 JST)
...
- **Q: なぜ active probe、passive signal (beaconRef health) じゃないのか?**
...
- **Q: なぜ visibility trigger が主、周期 backup が副なのか?**
  症状 1 の race が起きる唯一のタイミング = tab-hidden 復帰の ~1 秒窓。visibility
  をトリガーにすれば split 発生から数秒で検出できる。一方、network blip 等 visibility
  に乗らない経路もあり得るので 30s の汎用安全網を併設。視点を絞った主トリガーで
  速度、汎用 backup で漏れ防止、の分業。

- **Q: なぜ backup interval 30s なのか (15s じゃなく)?**
  probe 1 回で ~500ms-2s の network 負荷 (新 PeerJS インスタンス、STUN、ICE、
  WebRTC handshake、redirect 受信、destroy)。15s にすると負荷が倍。症状 1 は頻発
  するものではなく、主トリガー (visibility) で大半が拾えるので 30s で十分。

- **Q: 既存 `demoteToClient` との関係は?**
  既存は 2 段階 (§1006-1067): (a) `discoveryPm` 作って beacon に接続 →
  redirect で realHostId を discover、(b) 得られた realHostId で broadcast/connect/
  roleVersion++ の demotion。Stage 2 の probe は既に realHostId を手元に持っている
  (probe 自体が (a) を兼ねる) ので、(b) の末端処理のみ再利用。既存の (a) は Stage 2
  後も fail-count 由来の demotion として継続使用。
...
### Edge case (考慮したが現行設計で handle される)

1. **probe が自分自身の beacon に接続する**: 自分が legit holder なら `beaconRef`
   が redirect を送り返す (`actualHostId = myId`)。`redirect.hostId === myId` で
   OK 判定 → destroy。無害。
2. **probe 中に role が変わる (demote / migrate)**: `cancelled` フラグで in-flight
   probe を中断、cleanup 時 `probePm.destroy()`。新 role でも useEffect 再実行で
   新しい timer が再セットされる。
3. **probe 中に tab hidden になる**: 既存の `HOST_HIDDEN_GRACE` が peerManager
   自体を destroy。useEffect cleanup で probe も destroy される。復帰時に Phase 1
   から再起動するので、その時点で new probe が走る。orphan probe が残っても
   timeout (8s) で自浄。
4. **WS Relay mode**: `la-{roomName}` の beacon pattern を使わない。
   `if (activeTransport !== "peerjs") return;` で Stage 2 effect 全体を skip。
5. **solo host (client 0)**: probe は走るが `redirect.hostId === myId` で OK 判定。
   帯域微小 (30s に 1 回 + visibility 復帰時)、無害。client 0 check で skip しても
   良いが、solo → 1 client 追加の瞬間も split 耐性が欲しいので走らせておく方が
   対称的。
6. **PeerServer 不到達 (probe 失敗)**: `probePm.onPeerStatusChange` の error 分岐で
   destroy、timeout で保険。verification 不可 = assume OK で続行 (probe 失敗で
   demote すると逆に split を誘発する可能性)。次回 probe に期待。
7. **同時に 2 つの probe**: visibility 直後に 30s timer も fire するケース。2 つ
   並行走っても互いに独立、結果も同じ (legit か split か)。一方で split 検出
   されたら即 demote するので、もう一方が届いた頃には既に client になっていて
   no-op (isBeaconHolder=false で 2 個目の demote 試行がガードされる)。
...
## Stage 2 実装記録 & audit post-mortem — 2026-04-21 (`305d779` → `f8a4589`)

### 実装サマリ (`305d779`)

設計通り `PeerProvider.tsx` に:
- 定数 2 個 (`HOST_SELF_VERIFY_BACKUP_MS=30000` / `HOST_SELF_VERIFY_TIMEOUT_MS=8000`)
- top-level helper `performDemotion(pm, realHostId, onRoleChange)` — 既存
  `demoteToClient` 末端 5 ステップ (broadcast redirect → clearBeaconHolder →
  setBeaconHolderId → connect → roleVersion bump) を extract、Stage 2 probe と
  beacon-acquire の両 path で共有
- useEffect 1 つ (visibilitychange listener + 30s setInterval + initial probe
  on mount) 約 60 LOC

localhost 2 tab 検証: T1 (regression) / T3 (explicit split) / T4 (solo) OK。T2
(短 hide のつもり) で意図せず split 発生 → Stage 2 が自動解消するシーケンスが
観測された。

### 深掘り audit で発見した 4 bug (全て同日中に fix)

audit は Stage 2 実装直後に odakin 要請で 2 周実施。結果:

| # | bug | commit | 重大度 |
|---|---|---|---|
| 1 | 初回 probe 欠落: tab-hidden 復帰で effect cleanup が visibilitychange listener を一時撤去している間に復帰 event 発火 → 初回 split は 30s backup まで検出不能 | `13ebd64` | ⚠️ 実用影響あり |
| 2 | stale callback race: PeerJS の `peer.destroy()` は JS event loop の queued event を即 cancel しない → probe 1 の late callback が probe 2 を誤 destroy | `13ebd64` | ℹ️ defensive |
| 3 | self-demote guard 無し: beacon-acquire の `demoteToClient` は callsite で `realHostId !== myId` check 無し、stale PeerServer 登録に routing される rare race で self-redirect を受ける余地 | `235900f` | ℹ️ defensive |
| 4 | **LH 二重駆動 (catastrophic pre-existing)**: `assumeHostRole` は LH.ownerId を自分に書き換える (LH AI 駆動権取得) が、逆操作が `performDemotion` に無かった。demote 後も `lh.ownerId === myId` で `useGameLoop` の LH AI が走り続け、phaseSpace / laser を broadcast → 新 BH も同 LH を駆動 → LH 二重駆動 + registerHostRelay で他 peer にも二重配送 | `f8a4589` | 🚨 重大 |

### Bug 1 fix (`13ebd64` 初回 probe)

事前設計時の「visibility 復帰で probe」は、復帰→peerManager setState→effect
mount の順序で **listener 不在時に event が発火** するため不成立だった。**effect
mount 時に synchronous `runProbe()` を 1 回呼ぶ**ことで Phase 1 が race に勝って
作った split を即検出。副 trigger (30s backup) は blip 用に残す。

### Bug 2 fix (`13ebd64` stale guard)

`probePm !== pm` (local captured pm vs current outer) を 3 handler 冒頭 (timeout /
onPeerStatusChange / onMessage) に追加。destroy() semantics 依存を解消。

### Bug 3 fix (`235900f` self-demote guard)

`performDemotion` 冒頭に `if (realHostId === pm.id()) { console.warn(...); return; }`。
Stage 2 probe は callsite check 済だが beacon-acquire 側の保護にもなる。

### Bug 4 fix (`f8a4589` LH ownership 移譲 — 最重要)

`performDemotion` の `connect(realHostId)` 直後に、`assumeHostRole` の LH 更新 loop
と構造対称な逆操作を追加:
...
これは **既存 `demoteToClient` 経路でも同じ symptom だった pre-existing bug**。
Stage 2 で demote 頻度が上がって顕在化しやすくなった側面もあり、performDemotion
共通化で一箇所修正で両 path 救える形になった。

### Defer した既知の edge case (post-Stage 2)

- **Bug X: 3+ peer での disconnected peer resurrection**: Stage 1.5 で C が B を
  players に保持したまま A に snapshot 送信、A が B を re-add。Stage 3 (stale
  GC) で C 側が B を削除すれば解消。現スケール (2-peer 主) では顕在化せず defer。
- **Bug Y: LH.ownerId via stale client Stage 1.5 snapshot**: demoted client が
  snapshot 送信時に LH.ownerId preserve、受信側 local-newer check が pos.t 比較
  のみのため narrow race で stale ownerId が採用される余地。`76ba182` の sender-
  side fix で相当部分カバー済、受信側 authority check は追加 complexity に見合わず
  defer。
- **Partition loop**: A-B 完全 partition (A は PeerServer 到達可、A-B data channel
  不可) で B 側 migration → Stage 2 probe → demote → reconnect 失敗 → 再 migration
  のループ。Stage 2 特有ではなく beacon-acquire 側の既存 loop (MAX_BEACON_RETRIES
  起点) と同質、新規 regression ではないため accept。
