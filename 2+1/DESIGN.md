...
**4 geometry は observer 依存で毎 tick 再生成**: surface (`N × 2` 頂点 triangle strip) / 垂直線 (`N × 2` 頂点 LineSegments) / 過去光円錐交線 (`N_cone` 頂点 LineLoop) / 未来光円錐交線 (同 LineLoop)。`N = ARENA_RADIAL_SEGMENTS = 64`、`N_cone = ARENA_PAST_CONE_SEGMENTS = 128`。合計 ~600 頂点 × 毎 tick 再生成、BufferGeometry allocation コストは軽微。
