...
- **What**: リスポーン位置の座標時刻 t を、全プレイヤーの `phaseSpace.pos.t` の最大値に設定
- **Why**: 最先端のプレイヤーと同時刻にリスポーンすることで、即座に相互作用可能。因果律の守護者に引っかかることもない
- **History**: ホスト時刻 → maxT → `(minT + maxT) / 2`（`36abf67`）→ maxT に再変更。midpoint は「全員との時間差最小化」の意図だったが、1v1 で約 5 秒過去にリスポーンするため体感上ラグに見える問題があった
