...
**接平面の幾何**: 観測者を原点とする表示座標で交点 P=(x, y, t)、ρ=|xy|。過去 (t<0) / 未来 (t>0) 共通で、光円錐 F = x² + y² − t² = 0 の勾配から外向き単位法線は `n = (x, y, -t) / (ρ√2)`。レーザー xy 方向 `ℓ = (ℓx, ℓy, 0)` を接平面へ射影 → `u = ℓ − (ℓ·n)·n` を正規化、`v = n × u` で右手系を閉じ、[u|v|n] を group の rotation quaternion として設定。1 本の数式で過去/未来どちらも正しく扱える。
...
**実装**: `computeConeTangentQuaternion` ヘルパー (SceneContent.tsx) が過去/未来両方の render loop で共通利用。
