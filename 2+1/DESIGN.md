...
**動機**: 長時間プレイで laser (MAX 1000)・frozen WL (MAX 20)・debris (MAX 20) が上限近くまで溜まると、`SceneContent.tsx` useMemo の per-frame 全走査 (交差計算) + `DebrisRenderer` / `WorldLineRenderer` の per-object instance 更新コストが積み上がる。二分探索化で history 走査は O(log N) に落ちたが、object 数 N_obj 自体を削るともう一段下げられる。time fade が fade ≈ 0.04 で実質不可視 (@ Δt = 5×LCH) の領域にある object は視認価値ゼロなので GC してよい。

**判定**:

```
earliestPlayerT = min over players (LH 含む) of phaseSpace.pos.t
cutoff = earliestPlayerT − LIGHT_CONE_HEIGHT × GC_PAST_LCH_MULTIPLIER   // = -100 @ default
```

各 object の「最未来点」が cutoff 未満なら削除:

| object | 最未来点 |
|---|---|
| laser | `emissionPos.t + range` |
| frozen worldline | `history[last].pos.t` (= 死亡時刻) |
| debris | `deathPos.t + DEBRIS_MAX_LAMBDA` (= deathPos.t + 2.5) |

**実装** (`useGameLoop.ts` の tick 末尾): 3 種を逐次 filter して differing なら `store.set*` で更新。通常 cap が先に効く状況では GC は無作業 (フィルタ通過率 100%)。長時間プレイでのみ発火。

**定数**:
- `GC_PAST_LCH_MULTIPLIER = 5`: 閾値。下げすぎると「撃ったばかりの laser が見える範囲で消える」症状、上げすぎると削減効果減。5×LCH (= fade ≈ 0.038) が安全域。
- `DEBRIS_MAX_LAMBDA = 2.5`: `DebrisRenderer.tsx` の particle 長さ (従来 inline 定数) を extract。GC 判定と renderer で参照。

**安全性**: earliestPlayerT は LH 含む全 player の最小 coord time。新 joiner (低 t で参入) がいれば cutoff は自動的に後退するので、snapshot 受信時点で有効な object が GC されない。LH が respawn 済みの遠未来 t にいても、自機が低 t にいれば自機視点で削られない。

### Spawn effect の `depthWrite={false}` (2026-04-17 夜)

**症状**: 自機初回スポーン時、spawn pillar/ring が fade out する最終フレーム付近で「四角い穴」が一瞬見え、後ろの光円錐が欠ける。

**原因**: `SpawnRenderer.tsx` の pillar (CylinderGeometry) / ring (TorusGeometry) の `meshBasicMaterial` が `transparent=true` だが `depthWrite` の明示なし (default `true`)。opacity が fade で下がる間も pillar の mesh fragment が depth buffer に書き込まれ、後続の透明物 (光円錐 surface) が depth test で reject される → pillar があった矩形領域だけ後ろの光円錐が描かれない = 穴に見える。

**修正**: pillar / ring の material に `depthWrite={false}` 追加 (arena/光円錐/stardust/exhaust など他の透明物と統一)。透明物のソート順は three.js がカメラ距離で自動処理、depth を書かないことで互いに干渉しない。
...
| 定数 | 値 | 意味 |
|---|---|---|
| `STARDUST_COUNT` | 6000 | spark 総数。初期 1500 で前方流入が疎、4000 でも物足りず 6000 に (密度 ≈ 0.0035/world 単位³、spark 間隔 ≈ 6.6)。FPS ~65-70 (Apple Silicon) |
...
| `STARDUST_COLOR` | `hsl(42, 55%, 80%)` | 暖色 amber。初期 `hsl(40, 15%, 92%)` は彩度低すぎて LH `hsl(220,70%,75%)` の淡青と視覚混同、彩度上げて明確に分離 |
| `STARDUST_OPACITY` | 0.5 | base opacity (per-vertex time fade で乗算)。0.9 では前景に干渉、0.5 で背景として馴染む |

**描画** (`StardustRenderer.tsx`): 宣言的 `<points matrix={displayMatrix} matrixAutoUpdate={false} frustumCulled={false} renderOrder={-1}>` + `<pointsMaterial onBeforeCompile={applyTimeFadeShader}>`。useMemo で初回のみ `BufferAttribute` 確保、useFrame 毎 frame で wrap-around 検査 (N × 3 軸比較、recycling があれば `needsUpdate = true`)。

**timelike drift 実験と撤回** (2026-04-17 夜): spark.t を観測者.t の増分と同じ dt だけ進める案 (drift) を試した。意図は「世界系で静止した観測者に spark が止まって見える」= 各 spark が timelike worldline (β=0 の世界静止天体、案 16 star aberration skybox 寄り)。drift 版は初回デプロイ `2b6815b` に含めたが、体感評価で流入版 (drift なし = 純粋案 17 の null event) のほうが「時空 event を通過している感」が強く β 理念 (物理を通して体験) に沿うため、`b15694d` で drift ロジックを削除して流入版に戻した。

**world frame 時の fade 挙動** (既存 D pattern と同じ限界): `buildDisplayMatrix` が world frame で identity を返すため、shader の `modelMatrix × vertex` の z 成分は絶対 world t。観測者.t が進むと全 star が絶対 t 大きい側で薄くなる。stardust だけの問題ではなく、arena / worldlines / debris / lasers など全 D pattern renderer が world frame で同じ挙動。修正案: `buildDisplayMatrix` と `transformEventForDisplay` が world frame でも観測者並進を含む (= `T(-observer)` を返す) 設計への移行 → 全 D pattern renderer の world frame fade が rest frame と同質になる。本セッションでは stardust 固有の issue ではなく architectural question として defer (SESSION.md §次にやること)。

**所感**: デプロイ済み (`b15694d`、build `2026/04/17 23:27:38 JST`)。静止時に星屑が下方向に流れる + 加速時の光行差で前方集中 + rest/world 切替で見え方が変わる挙動が確認できる。残タスク: (a) 円形 alpha texture で点スプライトを丸く、(b) `PointsMaterial` の `additive blending` で重なり効果、(c) world frame fade 統一のための `buildDisplayMatrix` 修正 (広範囲影響)。
