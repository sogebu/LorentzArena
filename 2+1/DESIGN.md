...
### M17. 性能切り分けは「個別要素を段階的に α=0 にする二分法」
...
**中期対策 (未実装、defer)**: `physics/worldLine.ts` の交差系 API を**二分探索**で O(log N) 化。history は append-only で `pos.t` が単調増加 (tick 時系列順) なので、観測者 event との timelike separation が「最初に符号反転する箇所」を二分探索で特定可能 → API に下駄を履かせて呼び出し側は変更不要に。実装後は `MAX_WORLDLINE_HISTORY` を 5000 に戻せる (視覚的に長い世界線を保ちつつ FPS OK)。

**実装スケッチ (2026-04-17 試行、revert 済)**: `pastConeDistanceAt(i) = (observer.t − history[i].t) − ρ_i` と定義、通常は i について単調減少 (観測者時刻進行より空間移動が遅いため)。二分探索で `g(i) ≥ 0` の最大 i を特定、そこから ±K (= 16) の近傍だけ線形走査することで非単調ケース (観測者が高速で通過する等) にも tolerant になる。`futureLightConeIntersectionWorldLine` は対称的に `f(i) = history[i].t − observer.t − ρ_i` で `f(i) ≥ 0` の最小 i を探す。`findLaserHitPosition` は laser の時刻範囲 `[emissionPos.t, emissionPos.t + range]` に対応する segment index を `findLatestIndexAtOrBeforeTime` で両端 binary search して、その範囲だけ走査。試行は HMR 反映直後に「全描画消失」が起きて B 案のバグに見えたが、revert + full reload で正常復帰したため HMR Provider 再マウントの副作用と判定 (M15 事例)。B 案自体は動作未検証のまま revert、**再挑戦時はユニットテスト (linear scan 版と結果比較) を先に書く**方針。
...
**時間方向: observer.t 追従の window**: 無限円柱の時間方向スライスを描画したい。world 静止で固定した t 位置 (例 t=0) に anchor する案は観測者時間前進で画面外に流れる (§Spawn エフェクト pillar と同じ問題) ため却下。採用: 円柱 geometry は有限 `ARENA_HEIGHT = LIGHT_CONE_HEIGHT × 2 = 40`、mesh translation の t 成分は observer.t に追従。world 静止の time-translation invariant な空間構造 (z 軸に延びる円筒) を「常に観測者時間近傍の window」として描画する。円柱の geometry 自体は world frame で記述され D pattern で per-vertex Lorentz 変換を受けるので、rest frame では光行差で円→楕円に歪む (物理的に正しい)。

**H の選定と FPS トレードオフ**: 光円錐と同じ time span (= `LIGHT_CONE_HEIGHT × 2 = 40`) を採用。一度 400 (±200) まで延長して「観測者の光円錐の外まで含めた世界境界」を強めた試行もあったが、観測者が円柱の**外側から**眺めたとき、`DoubleSide transparent` の surface が画面の大部分を覆い、自機の光円錐 surface と合わせて 4 層以上の半透明 overdraw が走って pixel shader が fill-rate bound に (実測 FPS 10 まで低下、位置 (67, 52) で)。surface を削除して線だけにする案も試したが視認性が大幅に低下 → H を 40 に戻して surface を維持する方向に落ち着いた。**周期境界 (SESSION.md defer 中「アリーナ円柱の周期的境界条件 (トーラス化)」) で観測者が円柱外に出なくなれば、H を大きくしても FPS 問題は自動で消える**。その時点で H=400 レンジを再検討可能。GPU primitive 数は heightSegments = 1 のままなので不変 (Lorentz 変換は affine linear で側面は直線、中間セグメント不要)、Float32 精度も H=40 では十分 (~1e-6 精度)。

**過去光円錐 × 円柱 交線 (PastConeLoop)**: 観測者 (xo, yo, to) から見て、円柱上の各点 (cx + R cos θ, cy + R sin θ, t(θ)) が過去光円錐 t = to − √((x−xo)² + (y−yo)²) を満たす closed curve。128 サンプルの LineLoop を world frame で用意し matrix=displayMatrix で一括 Lorentz 変換。**各プレイヤーが自分の過去光円錐との交線を独立に描画** (`observerPos` は自機): 「今まさに光が届いている円柱の周縁」を明示し、world 静止円柱本体が薄く描画されていることを認知的に補完する。
...
**透明度と両面可視**: surface 0.08 (= 光円錐 surface と同値) / 垂直線 0.04 (= 光円錐 wireframe と同値) / PastConeLoop 1.0。surface は `THREE.DoubleSide` (光円錐と同じ扱い、カメラが円柱の内側にあっても裏面から側面が見える)。本体は極薄くして交線で「いま光が届いている円柱の周縁」を強調。

**surface 削除案を検討して却下**: 観測者が円柱外から見たときの overdraw FPS 対策として、surface を削除して垂直線 + 交線のみにする案も試したが、線だけでは円柱の「存在感」が失われて視認性が大きく低下。surface を残したまま H を 40 に抑えることで overdraw の画面占有率を抑制し、かつ視認性を維持する方向に落ち着いた。
