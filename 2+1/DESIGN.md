...
**時間方向: observer.t 追従の window**: 無限円柱の時間方向スライスを描画したい。world 静止で固定した t 位置 (例 t=0) に anchor する案は観測者時間前進で画面外に流れる (§Spawn エフェクト pillar と同じ問題) ため却下。採用: 円柱 geometry は有限 `ARENA_HEIGHT = 400` (= ± 200)、mesh translation の t 成分は observer.t に追従。world 静止の time-translation invariant な空間構造 (z 軸に延びる円筒) を「常に観測者時間近傍の window」として描画する。円柱の geometry 自体は world frame で記述され D pattern で per-vertex Lorentz 変換を受けるので、rest frame では光行差で円→楕円に歪む (物理的に正しい)。

**H = 400 の根拠 (初期案 H = 40 からの変更)**: 初期案は `LIGHT_CONE_HEIGHT × 2 = 40` (光円錐と同じ time span) だったが、観測者の光円錐の外まで円柱を延ばして「世界に常に存在する空間境界」の意味論を強めるため ±200 に拡大。GPU primitive 数は heightSegments = 1 のままなので不変 (Lorentz 変換は affine linear で側面は直線、中間セグメント不要)、頂点の Float32 精度も observer.x ±10 を足して ±210 で十分 (~1e-5 精度、~1e4 以上で破綻)。camera far clip plane を広げずに観測可能レンジが広がる。延ばしすぎ (~1e3 超) は depth buffer 精度 / z-fighting の懸念あり。
