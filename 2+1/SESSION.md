...
| 11 | **Network state loss / cascade chaos / sleep-wake** (= 5/5 verify session Phase 2-3 で多 tab cascade で観察 + 5/5 PM の sleep-wake で**reliable repro 確立**: 「両 client が因果律跳躍 同時 fire」 / 「LH worldline 縞 stack」 / `GL_INVALID_OPERATION` / 「一人の client から peer 不可視」 / 「両 tab がホスト化 + 信号サーバ復帰失敗で stuck」) | 🟢 **3 軸直交治療 + (a) layer 対称性完成 deploy 済 (5/5 evening、 build `18:54:11`)、 実機 verify 待ち** | 思想 anchor: [`design/network-recovery.md`](design/network-recovery.md) 6 軸 + 軸 2 layer 対称性 (= post-split phase が既存 design の盲点)、 plan v2: [`plans/2026-05-05-network-split-rule-b-runaway.md`](plans/2026-05-05-network-split-rule-b-runaway.md)。 真因 3 層 chain (H1 per-peer / H2 migration / H3 system) を 3 軸直交 fix で同時治療: **(e) reload prompt** (`712f2e2`、 escape hatch 軸) + **(a) markStale 拡張** (`0a6ea2f` WebRTC layer + `12ef1da` signaling layer = peer-unavailable、 per-peer 軸 layer 対称性完成、 H1 の 3 秒 unprotected window を 0-1 sec に圧縮) + **(d) PeerManager.reconnect** (`26dc8d7`、 system topology 軸、 H3 治療)。 (a) は 3 layer 独立 fault detector で全 layer 採用 (= WebRTC conn.close / heartbeat timeout 経由 disconnectPeer chain / signaling peer-unavailable)。 247 → 253 test pass、 typecheck clean。 odakin 実機 verify (sleep-wake / Network: Offline 5+sec) 待ち。 mesh-ish recovery は別 plan defer |
...
**Bug 11 (3 軸直交 fix + (a) layer 対称性完成 deploy 済 build `18:54:11`) — 実機 verify 待ち**:
