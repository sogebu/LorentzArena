...
deploy 後の追加 commit (= localhost only、 push 済 / deploy 未): `75aaae8` (test 追従) + `c7f7960` (Bug 7 fix) + **5/2 セッション後半 (= onboarding 系 / 因果律対称化 Stage 1-8 / dead-skip hotfix / WebGL context loss 根本対策 = 計 13 commits、 `a0f0bcc..453fca6`)**。 次回 deploy 時にまとめて反映、 deploy 候補。
...
| 10 | **全世界が固まる + 背景の星屑も止まる** (= rAF / WebGL レンダリングそのもの停止、 5+ 分プレイで顕在化、 odakin「元からあるバグ」 と確認) | ✅ **fundamental fix 済 (`453fca6`)** | **Root cause**: `WorldLineRenderer` の `useMemo` deps に `wl` を含めていたため `geoVersion = floor(wl.version/8)` の 8-tick throttle が事実上死に、 TubeGeometry rebuild (≈24000 vertex 構築 + GPU upload + 旧 dispose) が **毎 tick 60Hz** で走っていた。 alive 自機 + 他機 + LH = 3 worldLines × 60Hz = 180Hz の重 rebuild → main thread saturation → `setInterval` Violation 累積 (= odakin 観察で 16+) → rAF starve → 全世界 + 星屑凍結 → 最終 GPU 資源枯渇で WebGL Context Lost という連鎖。 **修正 (3 段)**: (1) `wl` を ref 経由で latest 参照 + deps から撤去 → rebuild 60Hz → 7.5Hz (1/8) に正規化。 (2) Canvas auto-remount on context loss: `canvasGeneration` increment で React が `<Canvas>` を unmount + mount → R3F が新 WebGL context 作成 → scene 再構築。 zustand store (= physics / killLog / score) は preserve、 user は 1-2 frame の flash で page reload 不要 (= invisible recovery)。 (3) Watchdog: 1.5s 内連続 loss は escape hatch overlay で「再読込」 UI。 設計思想: 「context loss を起こさない」 は外部要因 (OS/driver/power management) で不可能 → 「起きても気付かない」 に倒した |
...
**WebGL context loss 根本対策** ✅ **完了 (2026-05-02、 `63bf3f0` / `c14e1d5` / `453fca6`)** — odakin 観察「全世界凍結 + 星屑も止まる」 = Bug 10 の真因が `WorldLineRenderer` の `useMemo` deps 設計バグだったことを特定。 (1) `wl` を ref 経由で latest 参照 + deps 撤去で rebuild throttle 復活。 (2) Canvas `canvasGeneration` key で context loss を invisible auto-recovery。 (3) Watchdog で chronic loss は overlay escape hatch。 設計思想は「loss を起こさない」 → 「起きても気付かない」 に倒す。 別途 plan 起こしておらず本 SESSION + commit 内 docstring が単一 source of truth。
