...
- **DeadShipRenderer** 新設: SelfShipRenderer を仮想 player (pos=x_D, heading=heading_D) で wrap、useFrame で group traverse + material.opacity 上書き (transparent=true + depthWrite=false) で **ship 全体に opacity fade** を適用。opacity = `(DEATH_TAU_MAX − τ_0) / DEATH_TAU_MAX`。OtherPlayerRenderer の小 sphere path は廃止 (「model is the ship」の odakin 指摘)。OtherPlayerRenderer.tsx は 4 軸レビュー (2026-04-22) で dead code として削除。
