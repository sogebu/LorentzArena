...
- 3+1 次元への拡張検討
- **スマホ UI**: α（保守的）路線で着手予定。**詳細は [`EXPLORING.md`](./EXPLORING.md) の「スマホ UI の設計思考」参照**。2026-04-10 の設計議論で操作スキーム確定: 左に加速ボタン（W 相当）、空きエリアのスワイプで heading 回転、静止タッチで発射。描画 UI 要素は加速ボタン 1 つのみ。推力レバー（連続値、中間速度巡航）は検討課題。レーザーのエネルギー制（PC・スマホ共通）を検討中。MVP スコープ未決定
