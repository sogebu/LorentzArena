...
| 14 | **Background tab で physics runaway (= 寝ている間に未来に超高速 drift)** | 🟡 **真因未特定、 LH 経路 propagation は構造的遮断済 (5/6 plan)** | 2026-05-06 朝 user 報告: スマホ Brave で 8 時間 background 後、 世界時刻 `20340058.16s` (= 2034 万秒 = 235 日分、 wall_clock 8 時間で 700x speedup) / 位置 `(-19164502.22, 6810953.07)` = ±2000 万 ls 遠方 / 現速 0.3% c, γ=1.000 / 撃破数 odakin 28 / LH 3 で長 play 継続。 視覚: 中心から放射状光 (= 累積 worldline / laser worldline 多量) が異常密集。 真因仮説: (a) モバイル Brave background での setInterval throttle (= 1Hz minimum) で各 tick wall_dt が 1+ 秒になり pos.t 大 advance、 (b) [旧仮説 LH ↔ self ratchet は γ_max=1.89 cap で 700x speedup を作れないと判明、 棄却済]、 (c) frozen worldLines / debrisRecords / killLog の 8 時間蓄積で視覚異常密集。 **propagation race の防御進捗 (= 5/6 NPC 非対称 plan)**: (1) LH 経由の伝染 → **(I) NPC 非対称で構造的遮断完了** (= human の Rule B + spawn 計算で LH 除外、 LH runaway しても human 引っ張られない)、 (2) alive human runaway 経由の伝染 → **(II''') mean formula で partial defense** (= 1/N 重みで pull、 midpoint より robust) + 完全防御は L1 plausibility filter (別 plan) で対処、 (3) **(α) wall_clock anchor 案を永続却下** (= P1 設計柱矛盾)。 **残課題**: 真因 (= 700x speedup) の発生機構 isolation、 起きている間の再現実験 + L0 dTau cap + visibility listener 検討。 propagation race 自体は本 plan で大部分構造的に解消済 |
