...
**本番最新 deploy**: 2026-05-05 build `09:09:43` ([`e2608d1`](https://github.com/sogebu/LorentzArena/commit/e2608d1))。
...
| 12 | **灯台と自機の表示順序を取り合っている疑い** (= odakin 5/5 観察「灯台と自分の表示順序を取り合ってる気がする」) | ✅ **fix + deploy 済 (`e2608d1` build 09:09:43)、 user 実機 verify 待ち** | 真因: 旧 1 layer (renderOrder=10) で LH 全 12 mesh と self/rocket ship exhaust 4-2 nozzle が同 transparent group に混在 → sort tiebreaker (= back-to-front + material id) が float 精度で frame 間揺らぐ → 後描画側が visible 勝ちで flip = flicker。 修正: [`alwaysOnTopRender.ts`](src/components/game/alwaysOnTopRender.ts) を 2 layer 構造に拡張 (= BG renderOrder=10 / FG renderOrder=11)、 LH を BG、 self/rocket exhaust を FG に migration。 FG > BG = avatar 視認性優先で LH より上に exhaust 描画、 layer 間で決定論的 sort で flicker 消失見込み。 docstring drift (= 「適用先: 自機 hull / 砲塔」 と書いてたが実装は exhaust のみ) も同 commit で実態に sweep。 user 実機で「LH と自機 の表示順序揺らぎ」 が消えたか verify 待ち |
