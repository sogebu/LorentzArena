...
**本番最新 deploy**: 2026-05-05 build `09:23:54` ([`2e19da2`](https://github.com/sogebu/LorentzArena/commit/2e19da2))。
...
| 11 | **4+ tab beacon cascade chaos** (= 5/5 verify session Phase 2-3 で観察、 多 tab 同時運用 + host migration 連発で「両 client が因果律跳躍 overlay 同時 fire」 / 「LH worldline 縞 stack」 / 「`GL_INVALID_OPERATION` texture race」 / 「一人の client から peer 不可視」 が transient に出る) | **観察済 / repro 不安定 / 暫定 defer** | 安定 2-tab state では再現せず (Phase 4 確認)、 reliable repro 手順無し。 仮説: Fix B 2-sec cap (`MAX_VIRTUAL_TAU_SEC`) × network split で peer virtualPos が frozen → 両側 Rule B 一時 fire (= cascade transient phase 限定)。 LH worldline 縞 = host が pushFrozenWorldLine 連発 → 同 spatial 軸に複数 frozen entry stack → 重なり率の差で縞模様。 候補修正方向: (a) disconnect 検出時に peer を `staleFrozenIds` に追加 (= 既存 stale 機構 reuse、 Rule B / freeze から自動除外) / (b) Fix B cap を hard freeze → 線形 fall-off / (c) HUD 「接続中の相手」 表示 と peer 実状の整合化 (= disconnect なのに接続中表示の bug)。 un-defer trigger: 「stable 2-tab state でも再発」 or 「reliable repro 手順が見つかる」。 plan は repro 確立後に起草 |
| 12 | **灯台と自機の表示順序を取り合っている疑い** (= odakin 5/5 観察「灯台と自分の表示順序を取り合ってる気がする」) | ✅ **根本治療 deploy 済 (`2e19da2` build 09:23:54)、 user 実機 verify 待ち** | 当初 `e2608d1` で BG/FG layer 分離 (= 絆創膏)、 odakin 「絆創膏の上に絆創膏」 指摘で根本治療に切替。 真因: ALWAYS_ON_TOP pattern 自体が transparent な抽象可視化 material (= laser / 参照リング / debris marker × 2) が `depthWrite` default (= true) で書いてた offender への over-correction だった。 offender 側を `depthWrite={false}` に統一 → LH の depth bypass 不要 → ALWAYS_ON_TOP trio + BG/FG layer + module ごと撤去。 新原則: 「opaque 物理 entity は depth 書く、 transparent 抽象可視化 / 半物理 entity は depth 書かないが respect」。 LH ↔ Self ship hull が depth で前後決まる (= odakin 直感に整合)、 sort flicker は原理的に不発。 詳細 plan: [`plans/2026-05-05-depth-aware-cleanup.md`](plans/2026-05-05-depth-aware-cleanup.md) (= M26 application 学習記録、 「絆創膏 4 段重ねる前に offender audit していれば 1 commit で済んだ」 反省含む)。 |
| 10 | **全世界が固まる + 背景の星屑も止まる** (= rAF / WebGL レンダリングそのもの停止、 5+ 分プレイで顕在化、 odakin「元からあるバグ」 と確認) | 🟢 **主症状 ✅ + 並列 root fix deploy 済 (5/5 build 08:58:14)、 user 実機 verify 待ち** | 真因は **5 layer chain (5/4 撃滅) + 並列 root (5/5 撃滅)** の複合。 (1)-(5) chain (5/4): **virtualPos lastSync semantic 矛盾** (Fix A `dcd7469` + B `c8ef4b3`) / **LH Stage 4 implementation gap** (Fix C `b002d50`) / **frozenWorldLines mount storm** (`18adb8b` stable id) / **1 点 worldLine flicker** (`68e4f67`) / **myDeathEvent 二重管理** (`096f513`)。 (6) 並列 root (5/5、 `7a7df95`): **DebrisRenderer 毎 render allocation の GPU buffer upload 圧** が単独で Context Lost を生む経路 (Phase 1 単一 tab solo play 70 秒で Context Lost × 2 観察、 peer 不在 Rule B 不発で chain と独立)。 5/5 verify で主症状 (= 世界時刻 暴走 / 全世界凍結 / 死亡中 stardust 凍結) は user 「取れてるっぽい」 確認、 並列 root の Context Lost 頻度低下 + LH flicker after hit 消失 + setInterval Violation 累積低下を user 実機 verify 後に fully ✅。 5/2 fix (= renderer wlRef + Canvas auto-remount + watchdog) は二次防衛として温存。 |
...
- **全 renderer の useMemo deps + 毎 render allocation audit** — Bug 10 周辺で frozenWorldLines stable id + LH/OtherShip flicker fallback + StardustRenderer + DebrisRenderer (5/5) 確認は完了、 残部 (= LaserBatchRenderer / その他) を体系的 audit。 各 renderer で「`useMemo` deps に毎 tick 変わる object 参照を直入れ」「毎 render `new Float32Array` 等 GC pressure」 の 2 pattern を grep + ref pattern 標準化。 trigger: long plays で main thread saturation 系 symptom (= setInterval Violation / rAF starve / 視覚 stutter) が再発したら集中 audit。 副 candidate: DebrisRenderer の `explosionSegments` / `hitSegments` 配列毎 render 再生成 (~80 KB/render の CPU GC pressure、 5/5 fix の scope 外で残存)
