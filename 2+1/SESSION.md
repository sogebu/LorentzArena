...
deploy 後の追加 commit (= push 済 / deploy 未): `151bd84` (因果律跳躍 overlay) + `4061adb` (HUD「固有時間」 → 「世界時刻」 表記修正)。

**2026-05-04 セッション** (= 跳躍 overlay + 表記 fix + Bug 10 再発確認):
...
- **Bug 10 再発確認 (2026-05-04 plays)** — 5/2 fundamental fix (`453fca6`) 後にも user 実機で「また星屑が固まる」 を観察。 console に `THREE.WebGLRenderer: Context Lost` + `[Violation] 'setInterval' handler took <N>ms` × 8 (= 5/2 と同じ前兆症状)。 5/2 fix が WorldLineRenderer 単体の rebuild throttle のみ修復したため、 同じ pattern (毎 tick 変わる object 参照を `useMemo` deps に直入れ) が他の renderer に残存している可能性が高い。 Bug 10 ledger を「再発」 に格上げ、 次セッションで他 renderer 全 sweep + 同 pattern 探索が必要 (= grep `useMemo` の deps 全 audit)。 host migration 直後の race condition も別経路として疑い。
...
| 10 | **全世界が固まる + 背景の星屑も止まる** (= rAF / WebGL レンダリングそのもの停止、 5+ 分プレイで顕在化、 odakin「元からあるバグ」 と確認) | 🔴 **再発確認 (2026-05-04)** — 5/2 fundamental fix 部分的有効、 同 pattern 別箇所残存 | **5/2 Root cause**: `WorldLineRenderer` の `useMemo` deps に `wl` を含めていたため `geoVersion = floor(wl.version/8)` の 8-tick throttle が事実上死に、 TubeGeometry rebuild (≈24000 vertex 構築 + GPU upload + 旧 dispose) が **毎 tick 60Hz** で走っていた。 main thread saturation → `setInterval` Violation 累積 → rAF starve → 全世界 + 星屑凍結 → GPU 資源枯渇で Context Lost という連鎖。 **5/2 修正 (3 段)**: (1) `wl` を ref 経由 + deps 撤去で rebuild 60Hz → 7.5Hz。 (2) Canvas auto-remount: `canvasGeneration` increment で `<Canvas key>` 変化 → invisible recovery。 (3) Watchdog: 1.5s 内連続 loss で escape hatch overlay。 **5/4 再発**: user plays で「また星屑が固まる」 + console に Context Lost + setInterval Violation × 8 を観察。 **仮説**: 5/2 fix は WorldLineRenderer 単体修復のみ → 他の renderer (DebrisRenderer / OtherShipRenderer / LightConeRenderer / SpawnRingRenderer 等) に同じ「毎 tick 変わる object 参照を `useMemo` deps に直入れ」 pattern が残存し、 同種の rebuild storm が起きている可能性。 **次タスク**: 全 renderer の `useMemo` deps audit + 同 pattern を grep 検出 + ref 化 sweep (= `WorldLineRenderer` の wlRef pattern を全 renderer に展開) |
...
**WebGL context loss 根本対策** 🔴 **部分完了 + 再発確認 (2026-05-04)** — 5/2 (`63bf3f0` / `c14e1d5` / `453fca6`) で WorldLineRenderer 単体の `useMemo` deps 設計バグ + Canvas auto-remount + watchdog escape hatch を実装。 但し 2026-05-04 plays で同症状を user 再観察、 console にも同じ Context Lost + setInterval Violation が出ている。 5/2 fix は WorldLineRenderer に限定された patch であって全 renderer 横断の audit には至っていなかった、 が現時点の仮説 (= 同 pattern が他 renderer に残存)。 **次タスク**: `useMemo` deps に object 参照 (= 毎 tick 新参照になる Map / Array / WL state) を直入れしている全箇所を grep で audit、 wlRef pattern (= ref 経由 + deps から object 撤去) を全 renderer に展開。 設計思想 (= 「loss を起こさない」 → 「起きても気付かない」) は維持、 但し fundamental fix は audit + sweep 完了まで継続。
