...
**本番最新 deploy**: 2026-05-05 build `08:58:14` ([`7a7df95`](https://github.com/sogebu/LorentzArena/commit/7a7df95))。
...
| 12 | **灯台と自機の表示順序を取り合っている疑い** (= odakin 5/5 観察「灯台と自分の表示順序を取り合ってる気がする」) | **未着手 / 仮説段階** | 自機 (SelfShipRenderer) と LH (LighthouseRenderer) はどちらも [`alwaysOnTopRender.ts`](src/components/game/alwaysOnTopRender.ts) trio (= renderOrder=10 + depthTest=false + depthWrite=false) を持つ。 同 renderOrder + depthTest 無効で **sort tiebreaker が不安定** = 同 spatial 位置で重なる場面 (= 自機が LH 接近時 / 撃破時に同 pixel) で **z-fight 類似の flicker / 順序入れ替わり** が起きる可能性。 `9f711ca` の trio module 化時の暗黙前提「ALWAYS_ON_TOP entity 同士は同位置で重ならない」 が崩れる scenarioに対する設計 gap。 候補修正: (a) 自機 と LH で renderOrder を分離 (= LH=10、 自機=11 等) / (b) LH と 自機の同 pixel 重なりを物理的に避ける (= LH 撃破時 self ship を一時 hide、 等)。 odakin 5/5 verify session 中に observed、 詳細 RCA は別 session 候補 |
