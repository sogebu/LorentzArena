...
- `9f711ca` ALWAYS_ON_TOP render trio (renderOrder + depthTest + depthWrite) を共通 module 化 (= alwaysOnTopRender.ts)、 後の踏み外し防止
- `2cd8528` Lobby landscape を 3-col layout (form | ship | hi-scores) に再構成 + ship preview を center column 内 contained 配置
- `97ac479` Lobby landscape の ship preview 拡大 1 (= form/hi-scores 列幅 280→220 + ship min(70vh,280px) + camera 0.75× 寄せ)
- `e777e2e` Lobby landscape ship canvas をさらに拡大 (= 90vh/360 max + form/hi-scores 200px、 mobile 812×375 で ship 337×337 = viewport 90% 占有)
...
- **新 meta-principle M25-28** ([`design/meta-principles.md`](design/meta-principles.md)) — 5/4 セッションの方法論的知見を 4 原則として永続化:
  ...
  - **M28** (暗黙 trio / triad の踏み外し): LH flicker 事例 (= renderOrder=10 bump 時に depthTest=false が抜けて部分 frame 消失) から抽出。 「複数 prop が cohesive な pattern を成すとき (= 1 つ抜けで動作不定)、 個別 set 禁止、 共通 module の prop spread / factory に閉じ込めて踏み外し不可能化」、 user 「ad-hoc?」 指摘 → self-audit → trio module 化 (= [`alwaysOnTopRender.ts`](src/components/game/alwaysOnTopRender.ts)) の事例で「既存 pattern の不完全適用 (M28 application) vs 絆創膏 (M26 違反)」 の切り分けが M26 と並行 audit すべき独立 axis。 真因 audit (M24/25/26/27) ではなく**新規 entity 追加時の事前防衛**として運用。
  - 5 axis: M24 (鏡像 rule) / M25 (state 単一性) / M26 (patch 構造) / M27 (layer chain) は真因 audit の独立 4 axis、 M28 (暗黙 trio) は別系統 (= 既存 pattern 維持規律) で並列。 真因が見つからない時は前 4 axis から並行 audit、 新規 pattern application 時は M28 self-audit。
...
- **「ad-hoc?」 prudence trigger → trio module 化**: LH flicker fix 後 user「直し方 ad hoc ではない？大丈夫？」 で立ち止まり self-audit → 結論「**既存 pattern の不完全適用を完成させただけ**、 絆創膏 sign 無し」 + 「但し新規 entity 追加時の踏み外し risk は残る」 → **trio 共通 module 化** (`alwaysOnTopRender.ts`) で構造的固定 (= M28 application)。 教訓: user の「ad-hoc?」 question は M26 (絆創膏) と M28 (暗黙 trio 踏み外し) の両 axis で並行 audit すべき trigger、 「絆創膏ではない」 と判定しても「既存 pattern の不完全適用」 だった場合は **共通 module 化で踏み外し risk を構造的に潰す機会**。 「絆創膏ではない」 で safe 判定して終わらせない。
