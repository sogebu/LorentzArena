...
**本番最新 deploy**: 2026-05-05 build `18:31:59` ([`26dc8d7`](https://github.com/sogebu/LorentzArena/commit/26dc8d7))。

**5/5 evening Bug 11 連続実装 deploy** (= 思想 anchor 化 + 3 軸直交 fix 全部入り):
...
| 11 | **Network state loss / cascade chaos / sleep-wake** (= 5/5 verify session Phase 2-3 で多 tab cascade で観察 + 5/5 PM の sleep-wake で**reliable repro 確立**: 「両 client が因果律跳躍 同時 fire」 / 「LH worldline 縞 stack」 / `GL_INVALID_OPERATION` / 「一人の client から peer 不可視」 / 「両 tab がホスト化 + 信号サーバ復帰失敗で stuck」) | 🟢 **3 軸直交治療 deploy 済 (5/5 evening、 build `18:31:59`)、 実機 verify 待ち** | 思想 anchor: [`design/network-recovery.md`](design/network-recovery.md) 6 軸 (= post-split phase が既存 design の盲点)、 plan v2: [`plans/2026-05-05-network-split-rule-b-runaway.md`](plans/2026-05-05-network-split-rule-b-runaway.md)。 真因 3 層 chain (H1 per-peer / H2 migration / H3 system) を 3 軸直交 fix で同時治療: **(e) reload prompt** (`712f2e2`、 escape hatch 軸、 peerStatus 10sec watch で modal、 risk 0) + **(a) markStale 拡張** (`0a6ea2f`、 per-peer 軸、 conn drop で即時 stale 化、 H1 の 3 秒 unprotected window を ms 化) + **(d) PeerManager.reconnect** (`26dc8d7`、 system topology 軸、 peer.destroy + 新規作成、 H3 治療 / 既存 design 盲点を埋める)。 mesh-ish recovery (= 既存 stepping stone を migration recovery で初活用) は別 plan defer (= まず本 fix の実機 verify で sleep-wake stuck が解消するか確認)。 247 test pass、 typecheck clean。 odakin 実機 verify (sleep-wake / Network: Offline 5+sec) 待ち |
...
**Bug 11 (3 軸直交 fix deploy 済 build `18:31:59`) — 実機 verify 待ち**:
- **reliable repro**: PC sleep → wake (= 5/5 PM 確立済) または Chrome DevTools「Network: Offline」 5+ sec
- **(e) reload prompt** (`712f2e2`): peerStatus 10 sec disconnected/error 持続で「ネットワーク接続が失われました」 modal 表示、 reload button で復帰。 unavailable-id 等の room discovery transient は除外で false trigger 防止
- **(a) markStale** (`0a6ea2f`): peer disconnect 検出で staleFrozenIds に即時追加 → Rule B 暴走経路遮断、 因果律跳躍 同時 fire / LH worldline 縞 / setInterval Violation の 3 連症状が消えるか
...
- 期待挙動: sleep-wake で stuck せず自動復帰、 復帰失敗時は 10 sec で modal、 通常 play 時は visual 不変
