...
対戦可能。**`374972f` デプロイ済み** (build `2026/04/17 17:38:06 JST`)。本番 URL: https://sogebu.github.io/LorentzArena/
...
- **時間的距離 opacity fade (Lorentzian v1 per-vertex)** (2026-04-17): `fade = r²/(r² + Δt²)`、`r = TIME_FADE_SCALE = LIGHT_CONE_HEIGHT = 20` (時間距離の 2 乗反比例、物理の逆 2 乗法則と同型)。Δt = LCH で fade = 0.5、2×LCH で 0.2、3×LCH で 0.1 と緩やかに減衰。`game/timeFadeShader.ts` の `applyTimeFadeShader(shader)` を `onBeforeCompile` で material に inject (MeshStandardMaterial / MeshBasicMaterial / LineBasicMaterial、InstancedMesh も `USE_INSTANCING` 分岐で対応)。適用対象: WorldLineRenderer (生存・凍結)、DebrisRenderer (InstancedMesh)、自己光円錐 4 mesh、ArenaRenderer 4 mesh、LaserBatchRenderer。vertex の world 座標を modelMatrix で display frame に変換した z 成分から per-vertex fade を計算。per-mesh v0 (代表時刻で全体 scale) は廃止、`timeFade.ts` helper も削除。詳細は DESIGN.md §描画「時間的距離 opacity fade」
