...
**本番最新 deploy (実装中)**: 2026-05-16 **viewMode death dispatch** — クラゲ機体で自機死亡時、 沈下映像が SelfShipRenderer (= ガンシップ hull) で描画される regression が odakin 報告。 真因: [`DeadShipRenderer`](src/components/game/DeadShipRenderer.tsx) が viewMode 無視で SelfShipRenderer 固定。 修復: `viewMode?: "classic" | "shooter" | "jellyfish"` prop 追加 + 内部で OtherShipRenderer と対称な dispatch (= 旧 client / undefined は "classic" fallback)、 [`SceneContent.tsx`](src/components/game/SceneContent.tsx) の 2 call site (= 自機死亡 + 他機死亡) で `player.viewMode` を渡す。 traverse fade override は全 ship hull 共通で動作 (= group 内 Mesh を walk して material.opacity 上書き)、 thrust=0 + alpha4 未渡しで exhaust / arrow は非表示。 viewMode broadcast 拡張 (= 2026-05-07 [`2ffdfbc`](https://github.com/sogebu/LorentzArena/commit/2ffdfbc)) の dispatch 経路の漏れ。 typecheck + 285 test pass + build pass。
