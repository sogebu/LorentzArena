...
- **新 meta-principle M25** ([`design/meta-principles.md`](design/meta-principles.md) `M25. state の単一化原則`) — 上記 myDeathEvent decomposition から抽出した一般原則。 「derive 可能な state は explicit field と並存させない、 流入経路ごとに独立 set される drift / set 漏れ bug の温床」。 二重管理 pattern の構造説明 + decomposition 戦略 (= 静的部分 derive + 動的部分 explicit + lazy init で consumer 側補正) + 関連 M2/M3/M24 との関係。 audit で発見した同 class issue (= `player.isDead` の二重管理) を実例 2 として記載、 future plan へ link。
...
**[`plans/2026-05-04-isdead-decomposition.md`](plans/2026-05-04-isdead-decomposition.md)** 🟡 **plan only、 未着手** — myDeathEvent decomposition の audit で発見した同 class issue。 `RelativisticPlayer.isDead` field と `selectIsDead` derive の二重管理、 snapshot.ts L327 で強制同期 patch 既設。 真の解消は 25+ 箇所 read を全 derive 化 + field 削除、 reach 大で別 task。 trigger: long plays で「isDead drift 起因の visible 不整合」 観察、 or 大規模 protocol break refactor 機会。
...
- **`player.isDead` の二重管理解消** ([`plans/2026-05-04-isdead-decomposition.md`](plans/2026-05-04-isdead-decomposition.md)) — 2026-05-04 myDeathEvent decomposition の audit 発見。 同 class の二重管理 (= `RelativisticPlayer.isDead` explicit field と `selectIsDead` derive 並存)、 snapshot.ts L327 で「`derivedDead !== p.isDead` なら override」 と強制同期 patch 既設 = 二重管理の貼り絆 sign。 真の解消: 全 isDead read を `selectIsDead` 化 + field 削除、 reach 大 (= 25+ 箇所)、 別 task plan で集中して進める。 trigger: long plays で「isDead drift 起因の visible 不整合」 観察、 or 大規模 protocol break refactor 機会
