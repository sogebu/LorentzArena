...
対戦可能。**`c49ce40` デプロイ済み** (build `2026/04/20 18:36:03 JST`)。本番 URL: https://sogebu.github.io/LorentzArena/

マルチプレイ state バグ: **A (症状 3) / B (症状 2) / 症状 5** を修正 + deploy 済、
**タブ hidden 復帰 clock drift** も ballistic catchup で解消 (`c49ce40`)。
A は初版 deploy 後に再発、**新規接続 peer への intro unicast 再送** (`e9171c4`) で
真の root cause を解消。症状 5 は grace period 付き peer removal (`0066399`) で
players map 即時蒸発を防止。hidden 中も自機は coast (thrust なし、friction のみ)
していた semantic で phaseSpace を ballistic 積分、worldLine は freeze + 1 点 reset。
C (症状 1 + 4) は未着手。詳細: ↓ 2026-04-20 昼/午後/夕方 entry +
`plans/2026-04-20-multiplayer-state-bugs.md`。

2026-04-20 夕方 (タブ hidden 復帰 clock drift 解消) [`c49ce40`]:
- **問題**: 現設計は `dTau = visible wall delta` を各プレイヤーの proper time 刻みと
  して扱う (`evolvePhaseSpace` への入力)。`useGameLoop.ts:134` の hidden 分岐で
  `lastTimeRef.current = Date.now()` を fresh に保っていたため、hidden 中は pos.t が
  止まり、復帰後に他 peer との universal wall-clock 差が累積 (症状 5 grace period fix で
  「B が消えない」結果として drift が可視化されていた)。
- **fix**: hidden 分岐を単純 `return` に変更 (lastTime を fresh にしない)。復帰時の
  first tick は dTau = hidden 全体の wall delta として大きく、これを ballistic
  catchup (thrust=0, friction のみ、STEP=0.1s sub-step) で吸収。worldLine は
  messageHandler の gap reset と同じ semantic で freeze + 1 点 reset。
  catchup 後の phaseSpace は network にも送信し、他 peer 側は受信ハンドラの gap 検出で
  独立に自動 freeze + 新セグメント開始。
- **scope 外**: LH AI の catchup (host hidden 中は LH 発射 pause、UX 受け入れ)、
  ghost/dead 中の catchup (特殊経路のため現状維持)
- test: `ballisticCatchup.test.ts` 5 件 (stationary / 摩擦減衰 / 0 / 線形近似 /
  step boundary)。**51/51 pass**、typecheck clean、build clean

2026-04-20 午後 (症状 5 grace period 修正、未 deploy):
- **症状 5 (host migration & タブ復帰で相手消失) の仮説 (ii) に対応**: `RelativisticGame.tsx`
  切断 peer removal ロジックを即時削除から **`PEER_REMOVAL_GRACE_MS = 3000ms` の
  grace period 付き setTimeout** に変更。migration 中に一瞬 `connections` から落ちた相手を
  players map に残したまま復帰可能にする。猶予中に再接続したら `clearTimeout` でキャンセル、
  真に切れていたら猶予経過後に `setPlayers` で削除 + `stale.cleanupPeer` で stale refs purge。
- **新規**: `useStaleDetection` に `cleanupPeer(id)` helper 追加 (単一 peer の
  `staleFrozenRef` / `lastUpdateTimeRef` / `lastCoordTimeRef` 一括削除)。
- **unmount cleanup**: orphan setTimeout 防止のため、`pendingRemovalTimeoutsRef` の全
  timeout を解除する useEffect を追加。
- 値 3000ms の根拠: heartbeat timeout 2500ms (PeerProvider) を少し上回り migration race
  を吸収、ただし truly disconnected peer が長時間リスト残留しない程度。
- test: typecheck clean, 46/46 tests pass。Claude Preview は `document.hidden=true` で
  useGameLoop が止まるため 2 peer + reconnection は検証不能 → odakin 実機テスト依頼。
...
| 5 | **host migration & タブ復帰で相手が消える**: 切断 GC race で players map から蒸発 | **修正済**: grace period 付き removal (localhost 検証 OK、clock drift は別課題に defer) |
...
- ~~**タブ hidden 復帰時の clock drift**~~ → 2026-04-20 夕方 `c49ce40` で解消 (ballistic catchup、上記現在のステータス参照)
