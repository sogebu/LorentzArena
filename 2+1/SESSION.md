...
- 3+1 次元への拡張検討
- **スマホ UI**: α（保守的）路線で着手予定。**詳細は [`EXPLORING.md`](./EXPLORING.md) の「スマホ UI の設計思考」参照**。2026-04-10 の設計議論で操作スキーム確定: 左に加速ボタン（W 相当）、右に yaw スティック + fire ボタン。auto-fire・cruise control・pitch は不採用。S（後退）ボタンは MVP では省略、要否は実プレイで判断。レーザーのエネルギー制（PC・スマホ共通）を検討中。MVP スコープ未決定
