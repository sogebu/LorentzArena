...
- **タブ hidden 復帰時の clock drift** (2026-04-20 午後 発見、grace period fix で可視化):
  `useGameLoop.ts:134` で `document.hidden` 中は自分の pos.t を止める設計。復帰後、
  hidden 時間分だけ自分の pos.t が他 peer より遅れ、world frame で他 peer が z 正方向
  (未来) に浮いて見える。grace period fix が「B を消さない」結果として drift が可視化
  された (pre-existing、fix の副作用ではない)。選択肢: (a) 現状維持 (hidden = ローカル
  pause の物理的解釈)、(b) 復帰時に pos.t を wall-clock consensus に snap + WORLDLINE_GAP
  reset で連続化。Phase C (症状 1+4) の playerName primary key 化と一緒に設計議論予定
