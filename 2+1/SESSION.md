...
| 11 | **4+ tab beacon cascade chaos** (= 5/5 verify session Phase 2-3 で観察、 多 tab 同時運用 + host migration 連発で「両 client が因果律跳躍 overlay 同時 fire」 / 「LH worldline 縞 stack」 / 「`GL_INVALID_OPERATION` texture race」 / 「一人の client から peer 不可視」 が transient に出る) | **plan 起草済 / repro 待ち / defer** ([`plans/2026-05-05-network-split-rule-b-runaway.md`](plans/2026-05-05-network-split-rule-b-runaway.md)) | 安定 2-tab state では再現せず (Phase 4 確認)。 仮説 H1 (= 主、 信頼度 高): Fix B 2-sec cap (`MAX_VIRTUAL_TAU_SEC`) × network split で peer virtualPos が wall_clock に対して frozen → 両側 Rule B 一時 fire (= cascade transient phase 限定)。 仮説 H2: PeerProvider host election race。 候補修正 3 案 (= (a) staleFrozenIds 拡張 with disconnect callback / (b) Fix B cap soft fall-off / (c) HUD 整合化)、 推奨 (a) は既存機構 reuse の surgical 治療。 reliable repro 候補: Chrome DevTools「Network: Offline」 で 5+ 秒切断 → 再接続。 un-defer trigger 全 unmet で defer 継続、 但し plan に RCA + 候補実装 + 完了基準 文書化済 → reliable repro 確立次第 implementation phase に進める。 |
