...
- 🆕 **Bug 11 (network state loss) reliable repro 確立**: sleep computer → wake で 2 tab 共にホスト化 + 互いが見えない state 発生。 Tab 1 は signaling server の WebSocket 切断後 `Cannot connect to new Peer` で stuck (= 信号サーバ再接続経路の robustness 不足)、 Tab 2 は solo host で動作継続。 = 仮説 H2 (host election race) + 新発見 H3 (PeerProvider 再接続 robustness 不足) の合成症状。 plan の un-defer trigger 達成、 implementation phase 着手判断待ち
