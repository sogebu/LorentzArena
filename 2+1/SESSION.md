...
対戦可能。ホストマイグレーション実装済み（未テスト・未デプロイ）。モバイルタッチ入力・エネルギー制・因果律スコア・射撃グロウ・トグルスイッチ HUD を含む。
...
ホスト切断時に最古参クライアントが自動昇格し、ゲームを継続する仕組み。PeerJS / WS Relay 両方対応。

- **選出ロジック**: peerList の順序（接続順）で決定論的に選出
- **PeerJS**: 新ホストが残りクライアントに直接接続（旧ホストの `la-{roomName}` ID は再取得しない）
- **WS Relay**: `promote_host` でサーバー側ルーム再構築、他クライアントは `join_host` で合流
- **状態引継ぎ**: `hostMigration` メッセージでスコア + dead players + deathTime を送信
- **respawn タイマー**: `deathTimeMapRef` で kill 時刻を記録し、新ホストが残り時間で再構築
- **proactive peerList**: ホストが接続変化時に全クライアントへブロードキャスト（選出の前提）
- **ハートビート方式**: WebRTC ICE タイムアウト（30秒+）に依存せず、ホスト→クライアントの 3 秒間隔 ping + 8 秒タイムアウトで即時検知
- **stale 接続クリーンアップ**: `disconnectPeer()` で旧ホストの DataConnection を明示的に除去
- 設計判断は DESIGN.md「ホストマイグレーション」セクションに記録
- **テスト済み**: localhost 3 タブテストで PeerJS ホストマイグレーション動作確認（ホスト閉じ → 8 秒で検知 → 最古参昇格 → 残りクライアントに接続 → hostMigration ブロードキャスト → UI 更新）
