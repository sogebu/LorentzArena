...
- **マルチプレイ state バグ A + B 修正** (2026-04-20 昼、`2be56b4` + `8ce595f`): (A) `displayNames` を reactive state に昇格 + ControlPanel 4 段 fallback + applySnapshot の local/remote merge で peer ID prefix 露出を解消。(B) LH / 他機 dead branch の spawnT を `respawnTime.ts §getLatestSpawnT` 経由に差し替え、gap-reset で `worldLine.history[0]` が書き換わっても spawnT jump しないように。残 (C = 症状 1 host split + 症状 4 ghost 張り付き) は `plans/2026-04-20-multiplayer-state-bugs.md`
...
A / B は 2026-04-20 昼に修正 + deploy 済。残る症状 1 (host split) と症状 4
(ghost 張り付き) は reconnection で peerId が変わる時の ID 同一性問題で設計
変更が要る。3 案 (localStorage に peerId 永続化 / playerName を primary key 化
/ migration 確実化) は `plans/2026-04-20-multiplayer-state-bugs.md §C` に
列挙済み、方針選択から議論。

同 plan に **B'** (OtherPlayerRenderer LIVE branch の respawn 消失) も記録。
B は dead branch と LH の過去光円錐 visibility を直したが LIVE branch は
past-cone check していないので別原因、C の調査時に併せて再観測。
