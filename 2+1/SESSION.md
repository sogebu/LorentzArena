...
**本番最新 deploy**: 2026-05-05 build `19:15:56` ([`48f17d2`](https://github.com/sogebu/LorentzArena/commit/48f17d2))。
...
| 11 | **Network state loss / cascade chaos / sleep-wake** (= 5/5 verify session Phase 2-3 で多 tab cascade で観察 + 5/5 PM の sleep-wake で**reliable repro 確立**: 「両 client が因果律跳躍 同時 fire」 / 「LH worldline 縞 stack」 / `GL_INVALID_OPERATION` / 「一人の client から peer 不可視」 / 「両 tab がホスト化 + 信号サーバ復帰失敗で stuck」) | ✅ **plan fully closed: 3 軸直交治療 + (a) layer 対称性 + (c) HUD UI 全 deploy 済 + production verify confirmed (5/5 evening、 build `19:15:56`)** | 思想 anchor: [`design/network-recovery.md`](design/network-recovery.md) 6 軸 + 軸 2 layer 対称性。 真因 3 層 chain (H1 per-peer / H2 migration / H3 system) を 3 軸直交 fix + UI 整合化で完全治療: **(e) reload prompt** (`712f2e2`、 escape hatch 軸、 10sec disconnected で modal) + **(a) markStale 拡張** (`0a6ea2f` WebRTC layer + `12ef1da` signaling layer、 layer 対称性完成、 H1 の 3 秒 unprotected window を 0-1 sec に圧縮) + **(d) PeerManager.reconnect** (`26dc8d7`、 5sec disconnected で peer.reconnect → 失敗で destroy + 新規作成、 H3 治療) + **(c) HUD「接続中の相手」 3 状態 UI** (`48f17d2`、 connected/stale/disconnected で staleFrozenIds 可視化)。 production verify で `[PeerProvider] Signaling disconnected 5 sec — attempting PeerManager.reconnect()` + `[PeerManager] Attempting peer.reconnect()` log 観察、 自動復帰でゲーム継続。 plan §8 完了基準 全 ✅、 plan v2 status: closed。 247 → 253 test pass。 残課題 (= 別 task defer): localhost Vite HMR full reload (dev only) / host tab の「la-{room} 接続準備中/失敗」 残骸 entry の UI cleanup / mesh-ish recovery (= un-defer trigger 未達) |
...
**Bug 11 (3 軸直交 fix + (a) layer 対称性完成 deploy 済 build `18:54:11`) — production sleep-wake verify ✅ confirmed**:
